...
const GuestDestinationsComponent = ({ data }) => {
  ...
  const fetchPage = async (page) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/v1/destinations?page=${page}`);
      const newData = await response.json();

      setDestinations(newData.destinations || []);
  ...
  return (
    <main className={styles.container}>
      <div className={styles.headerSection}>
        <h4 className={styles.subheading}>Explore Experiences</h4>
      </div>
      ...
      {destinations.length === 0 ? (
        <div className={styles.emptyState}>
          ...
          <p>Check back later for new travel adventures.</p>
        </div>
      ) : (
                  ...
                  <div className={styles.priceSection}>
                    ...
                    <div className={styles.priceRow}>
                      <span className={styles.price}>
                        {destination.currency} {destination.start_price}
                      </span>
                    </div>
                  </div>
      ...
      {loading && <p className={styles.loading}>Loading destinations...</p>}
    </main>
  );
};

export default GuestDestinationsComponent;
